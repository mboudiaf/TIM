...
eval_parallel_ingredient = Ingredient('eval_parallel')
@eval_parallel_ingredient.config
def config():
...
class Evaluator:
    @eval_parallel_ingredient.capture
    ...
    @eval_parallel_ingredient.capture
    def run_full_evaluation(self, model, model_path, model_tag, save_plt_info, shots, method, callback):
        ...
        for shot in shots:
            ...
            if save_plt_info:
                if self.ex.current_run.config['tim']['finetune_encoder']:
                    method += '_all'
                    logs = {key: np.array(value) for key, value in logs.items()}
                    save_pickle(get_logs_path(model_path=model_path, method=method, shot=shot), logs)
